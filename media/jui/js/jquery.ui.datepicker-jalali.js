/*!
 * jQuery UI Datepicker Override 1.8.23
 *
 * Depends:
 *	jquery.ui.core.js
...
$.datepicker._autoSize= function(inst) {
		if (this._get(inst, 'autoSize') && !inst.inline) {
			var date = new JalaliDate(2009, 12 - 1, 20); // Ensure double digits
...
$.datepicker.iso8601Week= function(date) {
		var checkDate = new JalaliDate(date.getTime());
...
$.datepicker.parseDate= function (format, value, settings) {
		...
		shortYearCutoff = (typeof shortYearCutoff != 'string' ? shortYearCutoff :
				new JalaliDate().getFullYear() % 100 + parseInt(shortYearCutoff, 10));
			...
			if (literal)
				...
				switch (format.charAt(iFormat)) {
					...
					case 'y':
						...
						break;
					case '@':
						var date = new JalaliDate(getNumber('@'));
						year = date.getFullYear();
						month = date.getMonth() + 1;
						day = date.getDate();
						break;
					case '!':
						var date = new JalaliDate((getNumber('!') - this._ticksTo1970) / 10000);
						year = date.getFullYear();
						month = date.getMonth() + 1;
						day = date.getDate();
						break;
		...
		if (year == -1)
			year = new JalaliDate().getFullYear();
		else if (year < 100)
			year += new JalaliDate().getFullYear() - new JalaliDate().getFullYear() % 100 +
		...
		var date = this._daylightSavingAdjust(new JalaliDate(year, month - 1, day));
					...
					switch (format.charAt(iFormat)) {
						...
						case 'o':
							output += formatNumber('o',
								Math.round((new JalaliDate(date.getFullYear(), date.getMonth(), date.getDate()).getTime() - new JalaliDate(date.getFullYear(), 0, 0).getTime()) / 86400000), 3);
							break;
						case 'm':
...
$.datepicker._getDefaultDate= function(inst) {
		return this._restrictMinMax(inst,
			this._determineDate(inst, this._get(inst, 'defaultDate'), new JalaliDate()));
	};
...
$.datepicker._determineDate= function(inst, date, defaultDate) {
		...
		var newDate = (date == null || date === '' ? defaultDate : (typeof date == 'string' ? offsetString(date) :
			(typeof date == 'number' ? (isNaN(date) ? defaultDate : offsetNumeric(date)) : new JalaliDate(date.getTime()))));
...
$.datepicker._setDate= function(inst, date, noChange) {
		...
		var newDate = this._restrictMinMax(inst, this._determineDate(inst, date, new JalaliDate()));
...
$.datepicker._getDate= function(inst) {
		var startDate = (!inst.currentYear || (inst.input && inst.input.val() == '') ? null :
			this._daylightSavingAdjust(new JalaliDate(
...
$.datepicker._generateHTML= function(inst) {
		...
		today = this._daylightSavingAdjust(
			new JalaliDate(today.getFullYear(), today.getMonth(), today.getDate())); // clear time
		...
		var currentDate = this._daylightSavingAdjust((!inst.currentDay ? new JalaliDate(9999, 9, 9) :
			new JalaliDate(inst.currentYear, inst.currentMonth, inst.currentDay)));
		...
		if (maxDate) {
			var maxDraw = this._daylightSavingAdjust(new JalaliDate(maxDate.getFullYear(),
				maxDate.getMonth() - (numMonths[0] * numMonths[1]) + 1, maxDate.getDate()));
			maxDraw = (minDate && this._compareDate(maxDraw, '<', minDate) ? minDate : maxDraw);
			while (this._daylightSavingAdjust(new JalaliDate(drawYear, drawMonth, 1)) > maxDraw) {
				drawMonth--;
				if (drawMonth < 0) {
					drawMonth = 11;
					drawYear--;
		...
		prevText = (!navigationAsDateFormat ? prevText : this.formatDate(prevText,
			this._daylightSavingAdjust(new JalaliDate(drawYear, drawMonth - stepMonths, 1)),
		...
		for (var row = 0; row < numMonths[0]; row++) {
			...
			for (var col = 0; col < numMonths[1]; col++) {
				var selectedDate = this._daylightSavingAdjust(new JalaliDate(drawYear, drawMonth, inst.selectedDay));
				...
				var printDate = this._daylightSavingAdjust(new JalaliDate(drawYear, drawMonth, 1 - leadDays));
...
$.datepicker._generateMonthYearHeader= function(inst, drawMonth, drawYear, minDate, maxDate,
		...
		if ( !inst.yearshtml ) {
			...
			if (secondary || !changeYear)
				...
				var thisYear = new JalaliDate().getFullYear();
...
$.datepicker._adjustInstDate= function(inst, offset, period) {
		...
		var date = this._restrictMinMax(inst,
			this._daylightSavingAdjust(new JalaliDate(year, month, day)));
		inst.selectedDay = date.getDate();
		inst.drawMonth = inst.selectedMonth = date.getMonth();
		inst.drawYear = inst.selectedYear = date.getFullYear();
...
$.datepicker._getDaysInMonth= function(year, month) {
		return 32 - this._daylightSavingAdjust(new JalaliDate(year, month, 32)).getDate();
	};

$.datepicker._getFirstDayOfMonth= function(year, month) {
		return new JalaliDate(year, month, 1).getDay();
	};

$.datepicker._canAdjustMonth= function(inst, offset, curYear, curMonth) {
		var numMonths = this._getNumberOfMonths(inst);
		var date = this._daylightSavingAdjust(new JalaliDate(curYear,
...
$.datepicker._formatDate= function(inst, day, month, year) {
		...
		var date = (day ? (typeof day == 'object' ? day :
			this._daylightSavingAdjust(new JalaliDate(year, month, day))) :
			this._daylightSavingAdjust(new JalaliDate(inst.currentYear, inst.currentMonth, inst.currentDay)));
